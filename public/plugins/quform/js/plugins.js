...
;(function($) {
	var Quform = function($form, options) {
		...
		self.errorMessage = function (html, showTitle) {
			// Display the message
			var $message = $('<div className="quform-error-message"/>');

			if (showTitle !== false) {
				$message.append($('<div className="quform-error-title"/>').html(options.errorTitle));
			}
						...
						if (response.type === 'success') {
							...
							if (typeof options.success === 'function') {
								...
								var success = function () {
									...
									var $successMessage = $('<div className="quform-success-message"/>').html(response.message).hide().insertBefore(self.$container).fadeIn(options.successFadeInSpeed).show(0, function() {
										// successEnd function
								...
								$.each(response.elementErrors, function(index, info) {
									...
									if (typeof info.errors === 'object' && info.errors.length > 0) {
										...
										if ($element.length) {
											...
											var $errorList = $('<div className="quform-errors quform-cf"/>'),
											$errorWrap = $('<div className="quform-errors-wrap"/>').append($errorList);

											// Append the error
											$errorList.append('<div className="quform-error">' + info.errors[0] + '</div>');
